  ...
  <ul class="child-ul-wrapper" :class="float">
    <li v-for="i in itemchildren" :key="i.name" class="child-li-wrapper">
      <div v-if="i.children && i.children.length > 0" class="has-child">
        <div @click="parentBtn(i)" class="parent-name btn-wrapper-simple">
          <span class="nav-name-right">{{i.title}}==</span>
          <i class="icon"></i></div>
      ...
      <div v-else>
        ...
        <div @click.stop="fnHandler(i)" class="no-child-btn btn-wrapper-simple-noChild">
          <span class="nav-name-right nav-name-right-noChild">{{i.title}}</span>
        </div>
      </div>
    </li>
  </ul>
