<template>
  <ul class="vue-contextmenu-listWrapper" @contextmenu.stop=""
    :class="'vue-contextmenuName-' + contextMenuData.menuName">
    <li v-for="item in contextMenuData.menulists" class="context-menu-list" :key="item.title">
      <div v-if="item.children && item.children.length > 0" class="has-child">
        <div @click="parentBtn(item)" class="parent-name btn-wrapper-simple">
          <span class="nav-name-right">{{item.title}}++</span>
          <i class="icon"></i>
        </div>
        <tree-right :itemchildren="item.children" @childhandler="fnHandler" :float="floatDirection" />
      </div>
      <div v-else>
        <div @click.stop="fnHandler(item)" class="no-child-btn btn-wrapper-simple-noChild">
          <span class="nav-name-right nav-name-right-noChild">{{item.title}}</span>
        </div>
      </div>
    </li>
  </ul>
...
import treeRight from './Tree.vue'
export default {
  ...
  methods: {
    parentBtn (item) {
      item.one = '第一层'
      EventBus.$emit('keepOpen', item)
      console.log('父标签', item)
    },
    fnHandler (item) {
      console.log('没有子文件的时候。看这里', item)
      // this.$emit(item.fnHandler)
      EventBus.$emit('express_any_vue', item)
    }
  }
}
</script>
